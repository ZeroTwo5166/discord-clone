...
const DeleteServerModel = () => {
    ...
    useEffect(() => {
        if (server) {
            ...
            const adminRole = queryServer?.members.find((member:any) => member.role === "ADMIN");
            if (adminRole) {
            ...
            <Dialog open={showConfirmDialog} onOpenChange={() => {
                ...
                <DialogContent className="bg-white text-black p-0 overflow-hidden">
                    <DialogHeader className="pt-8 px-6">
                        ...
                        <DialogDescription className="text-center text-zinc-500">
                            Please type <span className="font-semibold select-none">"{confirmDeletionText}"</span> to confirm.
                        </DialogDescription>
                    </DialogHeader>
